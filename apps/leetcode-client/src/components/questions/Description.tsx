                ...
                ques?.examples?.map((example: string, index: number) => {
                  return (
                    <div key={index} className="">
                      <h3 className="font-semibold pb-1">Example: {index + 1} </h3>
                      <div className="flex flex-col pl-4">
                        <div className="flex gap-4">
                          {
                            Object.entries(example?.input).map((val, index) => {
                              return <p key={index}><span className="text-white font-semibold">Input: </span> {val[0]} = {JSON.stringify(val[1])} </p>
                            })
                          }
                        </div>
                        <p className=""><span className="text-white font-semibold">Output: </span> {JSON.stringify(example?.output)}</p>
                      </div>
                    </div>
                  )
                })
            ...
            <div className="flex flex-col gap-2">
              {
                ques?.constraints?.map((constraint: string, index: number) => {
                  return (
                    <span key={index} className="">{constraint}</span>
                  )
                })
              }
            </div>
